...
export default function PawsitivelySorry() {
  ...
  const audioRef = useRef<{ playMeow: () => void; startMusic: () => void }>(
    null
  );
    ...
    switch (step) {
      case "intro":
        return (
          <div className="flex flex-col items-center text-center animate-fade-in">
            <ShyCat />
            <h1 className="text-4xl md:text-5xl font-headline mt-4">
              Um... hey,
            </h1>
      ...
      case "slideshow":
        ...
        return (
          <div className="flex flex-col items-center justify-center text-center px-4 py-8 w-full h-full bg-cover bg-center rounded-2xl">
            <div
              className={cn(
                "bg-white/90 dark:bg-card/90 backdrop-blur-md p-6 sm:p-10 rounded-2xl shadow-2xl w-full max-w-2xl border-4 border-primary transition-none"
              )}
            >
              {currentSlide.image && (
      ...
      case "final":
        return (
          <div className="flex flex-col items-center text-center animate-fade-in">
            <CatWithBanner />
            <h1 className="text-3xl md:text-4xl font-headline mt-4">
              Will you forgive this foolish Boy?
            </h1>
            <p className="text-xl md:text-2xl mt-4 max-w-lg">
             Even the cutest cats mess up sometimes... but losing you hurts the most. Can we smile together again?
            </p>
            <Button
          ...
          <div className="flex flex-col md:flex-row items-center justify-center text-left w-full h-full max-w-6xl gap-6 px-4">
            ...
            <div className="md:w-1/2 w-full text-center md:text-left space-y-4">
              <h5 className="text-3xl font-extrabold font-headline text-pink-700 drop-shadow-lg">
                I know I mess things up sometimes, and yet, you always find it
              ...
              <p className="text-lg text-pink-500 italic mt-2 leading-relaxed">
                You're not just close to my heart — you're where I truly feel at
                home. <br />
                I’ll keep doing my best to love and cherish you more every
                single day. 🌸💖
              </p>
            </div>
            ...
            <div
              className="relative w-full md:w-1/2 h-[32rem] md:h-[40rem] animate-fade-in"
              style={{ animationDelay: "1s" }}
            >
              {apologySlides.map((slide, index) => {
                const layout = collageLayout[index % collageLayout.length];
                const sizeClass =
                  imageSizes[layout.size as keyof typeof imageSizes] ||
                  imageSizes.medium;
                return (
                  <div
                    key={index}
